...
BASE_MODEL= 'INCEPTION_RESNET_V2' #'VGG16' #'VGG16' # ['VGG16', 'RESNET50', 'InceptionV3', 'Xception', 'DenseNet169', 'DenseNet121']
...
EPOCH = 50
NUM_CLASS = 2
PATIENCE = 15 # parameters for early stop call back
...
train_TP_dir = os.path.join(DATA_DIR, 'bbox_patch_data/TP_bbox')
train_FP_dir = os.path.join(DATA_DIR, 'bbox_patch_data/FP_bbox')
train_TP_fps = glob.glob(train_TP_dir+'/'+'*.png')
train_FP_fps = glob.glob(train_FP_dir+'/'+'*.png')
train_TP = [{'id': file_name.split('_')[0], 'path': file_name, 'Target': 1} for file_name in train_TP_fps]
train_FP = [{'id': file_name.split('_')[0], 'path': file_name, 'Target': 0} for file_name in train_FP_fps]
train_data = train_TP + train_FP
image_bbox_df = pd.DataFrame(train_data)
image_bbox_df = image_bbox_df.sample(frac=1).reset_index(drop=True)
...
raw_train_df, valid_df = train_test_split(image_df, test_size=TEST_SIZE , random_state=2018, stratify=image_df['Target'])
#balance training dataset (validation set is not balance) and reduce the total image count
train_df = raw_train_df#.groupby('Target').apply(lambda x: x.sample(10000//NUM_CLASS)).reset_index(drop=True)
